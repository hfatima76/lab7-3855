...
def populate_stats():
    ...
    # result = session.query(Stats).order_by(...).first()
    # result = session.query(Stats).order_by(Stats.last_updated).first()
    ...
    events = requests.get(
        f"http://localhost:8090/buy?timestamp={last_updated}")
    events = events.json()
    for r in events:
        if r['item_price'] > result.max_buy_price:
            result.max_buy_price = r['item_price']
        result.num_buys += 1

    # TODO call the /sell GET endpoint of storage, passing last_update
    # call the /sell get enpoint of storage, passing the last update

    # TODO convert result to a json object, loop through and calculate max_sell_price of all recent records
    events = requests.get(
        f"http://localhost:8090/sell?timestamp={last_updated}")
    events = events.json()
    for r in events:
        if r['item_price'] > result.max_sell_price:
            result.max_sell_price = r['item_price']
        result.num_sells += 1
